...
#
class Node:
    def __init__(self,val,depth=None,left=None,right=None):
        self.val = val
        self.left = left
        self.right = right
        self.depth = depth

def gen_tree_from_indexes(indexes):
    root = Node(1,1)
    queue = deque([root])
    #max_d = 1
    for l,r in indexes:
        ...
        if l!=-1:
            cur_node.left = Node(l,cur_node.depth+1)
            queue.append(cur_node.left)
            #max_d = max(max_d,cur_node.left.depth)
        if r!=-1:
            cur_node.right = Node(r,cur_node.depth+1)
            queue.append(cur_node.right)
            #max_d = max(max_d,cur_node.right.depth)
    return root#,max_d
#
def in_order_traverse(root):
    stack = []
    def traverse_left_most(root):
        while root:
            stack.append(root)
            root = root.left
    traverse_left_most(root)
    ans = []
    while stack:
        cur_node = stack.pop()
        ans.append(cur_node.val)
        if cur_node.right:
            traverse_left_most(cur_node.right)
    return ans
    # ans = []
    # def helper(root):
    #     if root is None:
...
def swap_subtrees(root,k):
    stack = [root]
    while stack:
        cur_node = stack.pop()
        if cur_node.depth%k==0:
    ...
    # swap_subtrees(root.left,k)
    # swap_subtrees(root.right,k)

def swapNodes(indexes, queries):
    #
    # Write your code here.
    #
    tree = gen_tree_from_indexes(indexes)
    #print(tree.left.val)
    ans = []
    for k in queries:
        swap_subtrees(tree,k)
        ans.append(in_order_traverse(tree))
    #print(ans)
    return ans
