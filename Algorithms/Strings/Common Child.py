...
def commonChild(s1, s2):
    dis = [[1]*len(s1) for _ in range(len(s2))]
    for c in reversed(range(len(s1))):
        if s1[c]!=s2[-1]:
            dis[-1][c] = 0
        else:
            break
    for r in reversed(range(len(s2))):
        if (s2[r]!=s1[-1]):
            dis[r][-1] = 0
        else:
            break
    for r in reversed(range(len(s2)-1)):
        for c in reversed(range(len(s1)-1)):
            if s2[r]==s1[c]:
                dis[r][c] = 1+dis[r+1][c+1]
            else:
                dis[r][c] = max(dis[r+1][c],dis[r][c+1])
    return dis[0][0]
